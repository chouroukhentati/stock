    ...
    try {
      ...
      // create new product
       let newProduct = new Product({
         ...
         costPrice,
         sellingPrice,
         amountAvailable:Product.amountAvailable+ history[history.length - 1].numberBought,
         user: req.user.id
       });
     ...
     instance.defaults.headers.common["inventory-app-token"] = req.header(
      ...
      callAxios("POST", `/products/${requiredProduct._id}`, {
        name,
        costPrice,
        sellingPrice,
        amountAvailable:floor(requireProduct.amountAvailable)+floor(history[history.length - 1].numberBought)
      });
...
router.put("/:purchaseId", authenticator, async (req, res) => {
  try {
    ...
    callAxios( "PUT", `/products/${requiredProduct._id}`, {
      amountAvailable:floor(requiredProduct.amountAvailable) +floor( history[history.length - 1].numberBought)
    } ) 
